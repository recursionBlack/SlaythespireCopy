...
# 每个遗物激活时的间隔，防止所有遗物同时激活
const RELIC_APPLY_INTERVAL := 0.5
const RELIC_UI = preload("res://scene/relic_handler/relic_ui.tscn")

@onready var relic_control: RelicControl = $RelicControl
...
func get_all_relics() -> Array[Relic]:
	var relic_ui_node := _get_all_relic_ui_nodes()
	var relics_array: Array[Relic] =[]
	
	for relic_ui: RelicUI in relic_ui_node:
